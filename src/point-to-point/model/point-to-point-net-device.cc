...
PointToPointNetDevice::TransmitStart (Ptr<Packet> p)
{
  ...
  if (result == false)
    {
      m_phyTxDropTrace (p);
      //std::cout<<"m_phyTxDropTrace"<<std::endl;
    }
  return result;
}
...
PointToPointNetDevice::Receive (Ptr<Packet> packet)
{
  ...
  // packet->RemovePacketTag (tosTag);
  // std::cout<<" TOS = " << (((uint8_t)tosTag.GetTos ())>>5)<<std::endl;
  // Ipv4Header h;
  // packet->RemoveHeader(h);
  // h.GetPayloadSize();
  // std::cout<<h.GetPayloadSize() <<"            "<< h.GetTos()<<std::endl;
  if (m_receiveErrorModel && m_receiveErrorModel->IsCorrupt (packet) ) 
  ...
  else 
    {
      ...
      // Ipv4Header h;
      // packet->PeekHeader(h);
      // std::cout<<"  PointToPointNetDevice::Receive  TOS=     "<< (uint32_t)(h.GetTos())<<std::endl;
      // SocketPriorityTag priorityTag;
      // packet->RemovePacketTag (priorityTag);
      // uint8_t priority =h.GetTos();
  ...
  uint16_t protocolNumber)
{
  ...
  // std::cout<<"m_macTxDropTrace"<<std::endl;
  m_macTxDropTrace (packet);
