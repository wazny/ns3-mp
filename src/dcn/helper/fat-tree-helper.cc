...
FatTreeHelper::FatTreeHelper(unsigned N)
{
  ...
  // m_cpFactory.SetTypeId ("ns3::QbbNetDevice");
}
	...
	for (unsigned j=0; j<numST; j++) { // For each subtree
		for(unsigned i=0; i<N; i++) { // For each edge
			for(unsigned m=0; m<N; m++) { // For each port of edge
				...
				NetDeviceContainer devices = InstallCpCp(eNode, hNode);
...
FatTreeHelper::InstallCpCp (Ptr<Node> a, Ptr<Node> b)
{
	...
	//Ptr<CpNetDevice> devA = m_cpFactory.Create<CpNetDevice> ();
	Ptr<NetDeviceContainer> devA = m_cpFactory.Create<NetDeviceContainer> ();
	//devA->SetAttribute("QueueEq",UintegerValue(100));
	//devA->SetAttribute("QbbThreshold",UintegerValue(500));
	devA->SetAddress (Mac48Address::Allocate ());
	a->AddDevice (devA);
	//Ptr<CpNetDevice> devB = m_cpFactory.Create<CpNetDevice> ();
	Ptr<NetDeviceContainer> devB = m_cpFactory.Create<NetDeviceContainer> ();
